...
#'@export
plotcountmatrix2 <- function(countsummary,cex=.6,cexsummary=.7,showsummary=TRUE,showinterp=TRUE,...) {
  ...
  # daily run plot
  CIlo <- countsummary$calcmatrix$Yd - 1.96*countsummary$calcmatrix$rowse
  CIhi <- countsummary$calcmatrix$Yd + 1.96*countsummary$calcmatrix$rowse
  rect(26+3*showinterp,(-.5),30+3*showinterp,(-dim(countsummary$claritymatrix)[1]))
  xx <- seq(-.5,(-dim(countsummary$claritymatrix)[1]),by=-1)
  polygon(c(26+3*showinterp+(4*CIlo/max(CIhi,na.rm=T)),rev(26+3*showinterp+(4*CIhi/max(CIhi,na.rm=T)))),c(xx,rev(xx)),border=NA,col=colors[2])
  # lines(26+3*showinterp+(4*countsummary$calcmatrix$Yd/max(countsummary$calcmatrix$Yd,na.rm=T)),seq(-.5,(-dim(countsummary$claritymatrix)[1]),by=-1),col="blue",lty=1)
  lines(26+3*showinterp+(4*countsummary$calcmatrix$Yd/max(CIhi,na.rm=T)),seq(-.5,(-dim(countsummary$claritymatrix)[1]),by=-1),col="blue",lty=1)
  # YdCDF <- cumsum(countsummary$calcmatrix$Yd)/sum(countsummary$calcmatrix$Yd)
  YdCDF <- cumsum(countsummary$calcmatrix$Yd)/countsummary$CI[2]
  YdCDFlo <- (cumsum(countsummary$calcmatrix$Yd) - 1.96*sqrt(cumsum(countsummary$calcmatrix$rowvar)))/countsummary$CI[2]
  YdCDFhi <- (cumsum(countsummary$calcmatrix$Yd) + 1.96*sqrt(cumsum(countsummary$calcmatrix$rowvar)))/countsummary$CI[2]
  lines(26+3*showinterp+(4*YdCDF),seq(-.5,(-dim(countsummary$claritymatrix)[1]),by=-1),col="blue",lwd=2)
  polygon(c(26+3*showinterp+(4*YdCDFlo),rev(26+3*showinterp+(4*YdCDFhi))),c(xx,rev(xx)),border=NA,col=colors[3])
